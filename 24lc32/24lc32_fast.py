# This module demonstrates use I2CTransaction to communicate with
# EEPROM memory. Direct use of I2CTransaction is more complicated,
# but requires fewer calls to the board, boosting the performance.
#
# Demo requires a popular 24LC32 EEPROM. Please note, that 24LC32 comes in
...
class Eeprom24lc32(object):
    ...
    def write(self, address, data):
        ...
        t = I2CTransaction()
        t.s()
        t.a(self.chip_write_address)
        t.w(address_word)
        t.w(data)
        t.p()
        r = self.device.i2c_run(t)
        if not r.is_success:
            raise RuntimeError(r.description)

    def read(self, address, count=1):
        """
        Read bytes from address using faster transaction API.
        ...
        t.s()
        t.a(self.chip_write_address)
        t.w(address_word)  # 2 bytes, operations
        t.rs()
        t.a(self.chip_read_address)
        t.r(count)
        t.p()
        r = self.device.i2c_run(t)
        if not r.is_success:
            raise RuntimeError(r.description)
        return r.get_read_data()
...
if __name__ == "__main__":
    ...
    dev.i2c_clock = ArduinoUnoTwi.I2CClock.F250k
